...
(function() {
	...
	CKEDITOR.plugins.add('mt-stylewrap', {
		beforeInit: function(editor) {
			// extend style class
			var styleProto = CKEDITOR.style.prototype;

			styleProto.applyToRange = CKEDITOR.tools.override(styleProto.applyToRange, function(applyToRange) {
				return function(range) {
					var def = this.getDefinition();
					if (def.wrap === true && this.type == CKEDITOR.STYLE_BLOCK) {
						editor.execCommand('wrapstyle', this);
					} else {
						applyToRange.call(this, range);
					}
				}
			});

			styleProto.removeFromRange = CKEDITOR.tools.override(styleProto.removeFromRange, function(removeFromRange) {
				return function(range) {
					var def = this.getDefinition();
					if (def.wrap === true && this.type == CKEDITOR.STYLE_BLOCK) {
						editor.execCommand('wrapstyle', this);
					} else {
						removeFromRange.call(this, range);
					}
				}
			});

			styleProto.checkElementRemovable = CKEDITOR.tools.override(styleProto.checkElementRemovable, function(checkElementRemovable) {
