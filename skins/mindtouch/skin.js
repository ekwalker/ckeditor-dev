...
(function() {
		var icons = ( 'about,bold,italic,strike,subscript,superscript,underline,bidiltr,bidirtl,' +
			'blockquote,copy-rtl,copy,cut-rtl,cut,paste-rtl,paste,bgcolor,textcolor,creatediv,' +
			'docprops-rtl,docprops,find-rtl,find,replace,flash,button,checkbox,form,hiddenfield,' +
			'imagebutton,radio,select-rtl,select,textarea-rtl,textarea,textfield-rtl,textfield,' +
			'horizontalrule,iframe,image,indent-rtl,indent,outdent-rtl,outdent,justifyblock,' +
			'justifycenter,justifyleft,justifyright,anchor-rtl,anchor,link,unlink,bulletedlist-rtl,' +
			'bulletedlist,numberedlist-rtl,numberedlist,maximize,definitiondescription,definitionlist,' +
			...
			'showblocks-rtl,showblocks,smiley,source-rtl,source,sourcedialog-rtl,sourcedialog,specialchar,' +
			'table,templates-rtl,templates,uicolor,redo-rtl,redo,undo-rtl,undo' ).split( ',' );

		// The workaround to use the sprite instead of plugins' icons
		// We just add icons with an empty path. In this case plugins won't register icon
		// and inline style of the icon will be empty so the icons.css will be used for the dev

		for ( var i = 0; i < icons.length; i++ ) {
			CKEDITOR.skin.addIcon( icons[ i ], '' );
		}
})();
